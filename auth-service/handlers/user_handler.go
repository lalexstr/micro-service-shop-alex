...
// GetUserRole - internal endpoint to check user role
...
// UpdateUserRoleInternal - internal endpoint to update user role
func (h *UserHandler) UpdateUserRoleInternal(c *gin.Context) {
	idStr := c.Param("id")
	id64, err := strconv.ParseUint(idStr, 10, 64)
	if err != nil {
		utils.JSONError(c, http.StatusBadRequest, "invalid id")
